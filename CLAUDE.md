...
- **Skills** (42): `/add-laptop`, `/add-price`, `/add-model-year`, `/update-psref`, `/compare-specs`, `/new-component`, `/deploy-check`, `/e2e-check`, `/commit` (with prefix validation), `/add-editorial`, `/refresh-analysis`, `/benchmark-update`, `/changelog`, `/hardware-update`, `/gpu-update`, `/data-migration`, `/accessibility-check`, `/release`, `/stale-ref-check`, `/github-prep`, `/add-benchmark`, `/bulk-benchmark-verify`, `/price-import-test`, `/ingest-review`, `/run-tests`, `/add-gpu`, `/add-linux-compat`, `/perf-budget` (bundle size regression check), `/sync-counts` (auto-update model counts in docs), `/visual-test` (screenshots, diffs, chart audit, regression), `/data-verify` (CLI validation, sync, regression snapshots), `/model-check` (data + rendering + charts + price sanity), `/audit perf|price|retailer|psref` (targeted subsystem audits), `/pre-push-check` (full CI validation before push: Prettier + lint + test + validate + build), `/update-community-health` (sync CHANGELOG/SECURITY/SUPPORT versions), `/dependency-upgrade` (evaluate and apply safe dep upgrades), `/prettier-fix` (fix all Prettier issues repo-wide), `/e2e-scaffold` (generate Playwright E2E test stubs), `/cross-file-audit` (verify model coverage across all 9 data files), `/changelog-validate` (cross-reference CHANGELOG vs git log), `/type-coverage-audit` (scan for `any` and type coverage metrics), `/generate-data-schema` (auto-generate docs from types.ts)
- **Agents** (24): `pricing-verifier`, `ui-reviewer`, `a11y-reviewer`, `perf-analyzer`, `test-writer`, `component-test-writer`, `editorial-reviewer`, `upgrade-path-reviewer`, `scoring-auditor`, `bundle-analyzer`, `docs-reviewer`, `code-readability-auditor`, `benchmark-data-reviewer`, `cross-lineup-comparator` (compare UI tie handling + score range + chart scaling), `price-trend-analyzer`, `sustained-perf-analyzer`, `dependency-auditor`, `license-compliance-auditor` (MIT compatibility check on transitive deps), `validation-reviewer`, `lighthouse-auditor` (Playwright-based performance + accessibility audit), `data-reviewer` (specs + completeness), `hardware-reviewer` (GPU data + guide content), `chart-reviewer` (chart quality + cross-lineup handling), `data-completeness-checker` (cross-file coverage matrix for all 9 data files)
- **Hooks** (31): SessionStart (9): clear port 3000 + auto-start dev server (combined), dependency health, doc freshness, community skills sync, stale .next cache, benchmark coverage, stale ref detector, skill freshness, community health file freshness; PostToolUse (14): ESLint, tsc, Prettier (broadened to .yml/.yaml/.mjs/.cjs/.js), data validation (`npm run validate`), validate-data.ts meta-check, context reminders (ScoreBar/GPU/laptops.ts/chart height), image size warning, data file checks (as-const/duplicate keys/BenchmarkSource), chart checks (use-client/Map-Set guard), Vitest runner, stale agent name, npm audit + dep count warning, build detection info, dev server restart after build; Stop (1): kill dev server; PreToolUse (7): security block (lock files + .env/.pem/.key), linter-maintained file warning, recharts import reminder, seed price sequential ID context, CHANGELOG reminder on version bump, git commit/push Prettier gate (blocking), Exa cost warning
