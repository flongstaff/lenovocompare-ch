...
// Count category wins per model
const getOverallWinner = (winners: CategoryWinner[], models: readonly Laptop[]) => {
  ...
  return { model: models[bestIdx], index: bestIdx, wins: bestWins, total: winners.length };
};

export const QuickVerdict = ({ models, prices }: QuickVerdictProps) => {
  const winners = getWinners(models, prices);
  const overall = getOverallWinner(winners, models);
  ...
  return (
    <div className="carbon-card rounded-lg p-4">
      ...
      <div
        ...
        style={{ background: "rgba(15, 98, 254, 0.1)", border: "1px solid rgba(15, 98, 254, 0.3)" }}
      >
        <div
          className="h-3 w-3 flex-shrink-0 rounded-sm"
          style={{ background: COMPARE_COLORS[overall.index % COMPARE_COLORS.length] }}
        />
        <div>
