...
- **Skills** (48): `/add-laptop`, `/add-price`, `/add-model-year`, `/update-psref`, `/compare-specs` (side-by-side spec and score comparison of 2-4 models in the terminal), `/new-component` (scaffold a new React component following project conventions), `/deploy-check`, `/e2e-check` (run Playwright E2E tests against the local dev server), `/commit` (conventional commits with type/scope validation, banned words, 50-char subject limit), `/squash-commits` (consolidate messy branch history into clean conventional commits), `/add-editorial`, `/refresh-analysis` (regenerate analysis outputs for one or all models), `/benchmark-update`, `/changelog`, `/hardware-update`, `/gpu-update`, `/data-migration`, `/accessibility-check`, `/release`, `/stale-ref-check`, `/github-prep`, `/add-benchmark`, `/bulk-benchmark-verify`, `/price-import-test`, `/ingest-review`, `/run-tests`, `/add-gpu`, `/add-linux-compat`, `/perf-budget` (bundle size regression check), `/bundle-regression` (delta tracking against persisted baseline per route), `/sync-counts` (auto-update model counts in docs), `/visual-test` (screenshots, diffs, chart audit, regression), `/data-verify` (CLI validation, sync, regression snapshots), `/model-check` (data + rendering + charts + price sanity), `/audit perf|price|retailer|psref` (targeted subsystem audits), `/pre-push-check` (full CI validation before push: Prettier + lint + test + validate + build), `/update-community-health` (sync CHANGELOG/SECURITY/SUPPORT versions), `/dependency-upgrade` (evaluate and apply safe dep upgrades), `/prettier-fix` (fix all Prettier issues repo-wide), `/e2e-scaffold` (generate Playwright E2E test stubs), `/cross-file-audit` (verify model coverage across all 9 data files), `/changelog-validate` (cross-reference CHANGELOG vs git log), `/type-coverage-audit` (scan for `any` and type coverage metrics), `/generate-data-schema` (auto-generate docs from types.ts), `/psref-audit` (validate PSREF URLs + compare processor configs against live pages), `/hook-health` (dry-run diagnostic for all hooks), `/add-deal` (add curated deal highlight or update market insights), `/test-component` (scaffold Vitest + @testing-library/react test for a component)
- **Agents** (30): `pricing-verifier`, `ui-reviewer`, `a11y-reviewer`, `perf-analyzer`, `test-writer`, `component-test-writer`, `editorial-reviewer`, `upgrade-path-reviewer`, `scoring-auditor`, `bundle-analyzer`, `docs-reviewer`, `code-readability-auditor`, `benchmark-data-reviewer`, `cross-lineup-comparator` (compare UI tie handling + score range + chart scaling), `price-trend-analyzer` (seed prices, baselines, retailer pricing, value opportunities), `sustained-perf-analyzer` (thermal throttling, sustained vs peak performance, battery-performance tradeoffs), `dependency-auditor`, `license-compliance-auditor` (MIT compatibility check on transitive deps), `validation-reviewer`, `lighthouse-auditor` (Playwright-based performance + accessibility audit), `data-reviewer` (specs + completeness), `hardware-reviewer` (GPU data + guide content), `chart-reviewer` (chart quality + cross-lineup handling), `data-completeness-checker` (cross-file coverage matrix for all 9 data files), `psref-config-comparator` (live PSREF page diff against laptops.ts configs), `regression-tester` (scoring stability verification against representative models), `deals-reviewer` (deal highlights, market insights, buy-signal accuracy), `price-freshness-checker` (detect stale prices older than 90 days), `count-drift-auditor` (detect stale numeric claims across all text-bearing files), `visual-regression-tester` (Playwright screenshot baselines and visual diff detection)
- **Hooks** (33): SessionStart (11): clear port 3000 + auto-start dev server (combined), dependency health, doc freshness, community skills sync, stale .next cache, benchmark coverage, stale ref detector, skill freshness, community health file freshness, PSREF data freshness, price freshness; PostToolUse (15): code quality (Prettier + ESLint + tsc + re-stage — consolidated), data integrity (validate + as-const/dupes/sources — consolidated), validate-data.ts meta-check, deals data (duplicate IDs + laptopId cross-check), context & patterns (ScoreBar/GPU/laptops.ts/charts/recharts/agents — consolidated), image size warning, Vitest runner, npm audit + dep count, PSREF scraper type-check, PSREF URL validation (prefix + slug cross-check — consolidated), build detection info, dev server restart after build, .next auto-clear on file delete/move, data integrity tests (Vitest on data/\*.ts edits), a11y static check (img alt, keyboard handlers, aria-labels, form labels); Stop (1): kill dev server; PreToolUse (7): security block (lock files + .env/.pem/.key), linter-maintained file warning, recharts import reminder, seed price sequential ID context, CHANGELOG reminder on version bump, git commit/push gate (Prettier check + commit message linter — consolidated), Exa cost warning
- **MCP**: context7 (live docs, plugin), memory (cross-session persistence, npx), playwright (local browser automation/E2E, npx), github (issues/PRs/actions, npx), firecrawl (web fetch/search, plugin), Figma/Slack/Hugging Face/Cloudflare (cloud integrations) — `.mcp.json` for local servers, plugins for cloud
- **CI/CD**: `.github/workflows/ci.yml` (lint + test + build), `.github/workflows/deploy.yml` (GitHub Pages deploy), `.github/workflows/codeql.yml` (CodeQL security analysis), `.github/workflows/process-price-submission.yml` (community price → PR on `price-approved` label), `.github/dependabot.yml` (weekly dependency updates)
...
- **`app/`** — Next.js App Router pages: home (grid), `/model/[id]` (detail), `/compare` (side-by-side), `/pricing` (manage prices), `/hardware` (CPU/GPU guide), `/deals` (market dashboard with buy/wait signals)
...
- **`data/laptops.ts`** — 124 Laptop models with real PSREF specs (2018–2025): 76 ThinkPad + 14 IdeaPad Pro + 18 Legion + 16 Yoga. Every model has `lineup` and `psrefUrl` (required). Aliased as `thinkpads` for compat.
...
- **`data/linux-compat.ts`** — Per-model Linux compatibility (certified distros, kernel, driver notes, Fedora notes) — all 124 models
- **`data/model-editorial.ts`** — Curated editorial for all 124 models
...
- **Lineups**: `"ThinkPad" | "IdeaPad Pro" | "Legion" | "Yoga"` — each model has a `lineup` field. Series values vary by lineup (ThinkPad: X1/T/P/L/E, IdeaPad Pro: Pro 5/Pro 5i/Pro 7, Legion: 5/5i/7/7i/Pro/Slim, Yoga: Yoga 6/Yoga 7/Yoga 9/Yoga Slim/Yoga Book)
