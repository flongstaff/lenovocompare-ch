...
const ThermalCompareChart = ({ models }: ThermalCompareChartProps) => {
  const data = [
    {
      metric: "Keyboard °C",
      ...Object.fromEntries(models.map((m, i) => [`model${i}`, getModelBenchmarks(m.id)?.thermals?.keyboardMaxC ?? 0])),
    },
    {
      metric: "Underside °C",
      ...Object.fromEntries(
        models.map((m, i) => [`model${i}`, getModelBenchmarks(m.id)?.thermals?.undersideMaxC ?? 0]),
      ),
    },
    {
      metric: "Fan dB",
      ...Object.fromEntries(models.map((m, i) => [`model${i}`, getModelBenchmarks(m.id)?.fanNoise ?? 0])),
    },
  ];

  const chartHeight = Math.max(140, models.length * 28 + 80);
