...
const METRICS = ["Office Hours", "Video Hours"] as const;
...
const BatteryLifeCompareChart = ({ models }: BatteryLifeCompareChartProps) => {
  ...
  const scores = METRICS.map((metric) =>
    models.map((m) => {
      ...
      if (metric === "Office Hours") return benchmarks?.battery?.officeHours ?? 0;
      return benchmarks?.battery?.videoHours ?? 0;
    }),
  );
  ...
  const xMax = Math.ceil(maxVal / 2) * 2;

  // X-axis ticks
  const tickStep = xMax <= 10 ? 2 : xMax <= 20 ? 4 : 5;
  const xTicks: number[] = [];
