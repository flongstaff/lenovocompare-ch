...
export const laptopRegistry: readonly LaptopSpec[] = [
  // ─── T14 Gen 6 (AMD) ─────────────────────────────────────────────
  {
    ...
    dealbreakers: [
      'DDR5 SODIMM – user-upgradeable RAM',
...
export const getBrackLink = (spec: LaptopSpec): string =>
  `${RETAILER_BASES.brack}${spec.pricing.brackSearchQuery}`;
...
export const getLaptopById = (id: string): LaptopSpec | undefined =>
  laptopRegistry.find((l) => l.id === id);
